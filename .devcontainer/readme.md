...
After modifyng the source in VCS, you have to go inside your container and use a command tool do build it.

```shell
docker exec -it sogo_dev bash
```

There is a command tool `devenv` with following options for building:
```
